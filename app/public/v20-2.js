  ...
  async function applyInstruction() {
    ...
    try {
      ...
      if(result.needsConfirmation){
        ...
        $('#v202-result').innerHTML=`<div class="v202-risk-preview ${escapeHtml(level)}"><div class="v202-risk-head"><span>⚠</span><div><b>Vista previa · Riesgo ${escapeHtml(riskLabel)}</b><p>${escapeHtml(result.summary||'La acción requiere confirmación antes de modificar el CRM.')}</p></div></div>${conflicts.length?`<div class="v202-conflicts"><b>${conflicts.length} conflicto(s) potencial(es)</b>${conflicts.slice(0,4).map(c=>`<small>• ${escapeHtml(c.ruleName||'Regla')}: ${escapeHtml(c.detail||c.type||'Conflicto')}</small>`).join('')}</div>`:''}<div class="inline-actions"><button id="v202-confirm-pending" class="button primary" type="button">${special?'Confirmación especial y aplicar':'Confirmar y aplicar'}</button><button id="v202-cancel-pending" class="button ghost" type="button">Cancelar</button></div></div>`;
        $('#v202-confirm-pending')?.addEventListener('click',()=>void confirmInstruction(result.pendingId,special));
        $('#v202-cancel-pending')?.addEventListener('click',()=>{$('#v202-result').innerHTML='';});
        return;
      }
      if(result.state) setState(result.state);
      const labels={create_rule:'Regla creada y activada',update_rule:'Regla modificada',toggle_rule:'Estado de regla actualizado',delete_rule:'Regla eliminada',execute_admin_actions:'Cambios aplicados al CRM'};
      const conflicts=(result.conflicts||[]).length?`<small>⚠ ${result.conflicts.length} conflicto(s) potencial(es) detectados.</small>`:'';
      $('#v202-result').innerHTML=`<div class="v202-success"><span>✓</span><div><b>${escapeHtml(labels[result.operation]||'Instrucción aplicada')}</b><p>${escapeHtml(result.summary||result.rule?.name||'Los cambios ya están activos.')}</p>${result.warning?`<small>⚠ ${escapeHtml(result.warning)}</small>`:''}${conflicts}</div></div>`;
      $('#v202-instruction').value=''; await loadSuper(); showToast(labels[result.operation]||'Super IA aplicada');
    } catch(e) { $('#v202-result').innerHTML=`<div class="v202-error"><span>!</span><div><b>No se pudo aplicar</b><p>${escapeHtml(e.message)}</p></div></div>`; showToast(e.message,'warning'); }
    finally { btn.disabled=false; btn.textContent='✦ Analizar / aplicar'; }
  }

  async function handleRuleAction(event) {
    const row=event.target.closest('[data-rule-id]'); const action=event.target.closest('[data-rule-action]')?.dataset.ruleAction; if(!row||!action)return;
    const id=row.dataset.ruleId;
    try {
      if(action==='toggle') await api(`/api/admin/super-automation/rules/${encodeURIComponent(id)}`,{method:'PUT',body:JSON.stringify({enabled:event.target.checked})});
      if(action==='delete') { if(!await confirmAction('Eliminar automatización','La regla dejará de ejecutarse y se cancelarán esperas activas vinculadas.'))return; await api(`/api/admin/super-automation/rules/${encodeURIComponent(id)}`,{method:'DELETE'}); }
      await loadSuper();
    } catch(e){ showToast(e.message,'warning'); await loadSuper(); }
  }

  async function saveSettings() {
    try { const next=await api('/api/admin/super-automation/settings',{method:'POST',body:JSON.stringify({enabled:$('#v202-enabled').value==='true',executeDirectly:true,silentByDefault:$('#v202-silent').checked,defaultReplyTimeoutMinutes:Number($('#v202-timeout').value||60),maxChainDepth:10,logExecutions:true})}); setState(next); await loadSuper(); showToast('Configuración de Super IA guardada'); }
    catch(e){ showToast(e.message,'warning'); }
  }

  function renderV202() {
    inject();
    if (!$('#v202-super-automation')) return;
    const isAdmin=adminOnly(); $('#v202-super-automation').hidden=!isAdmin;
    document.querySelector('[data-module-block="automationLab"]')?.toggleAttribute('hidden',true);
    const oldNatural=document.querySelector('#v201-natural-action')?.closest('article'); if(oldNatural) oldNatural.hidden=true;
    if (appState?.settings?.stageLabels) { try { Object.assign(stageLabels, appState.settings.stageLabels); } catch {} }
    if (isAdmin && currentView==='advanced') void loadSuper();
  }

  document.addEventListener('DOMContentLoaded',()=>{inject();renderV202();});
  const previousRenderAll=renderAll; renderAll=function(){ if(appState?.settings?.stageLabels){try{Object.assign(stageLabels,appState.settings.stageLabels)}catch{}} previousRenderAll();renderV202();};
  const previousSwitchView=switchView; switchView=function(view){previousSwitchView(view); if(view==='advanced'&&adminOnly())void loadSuper();};
})();
