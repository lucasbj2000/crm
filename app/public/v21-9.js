...
(()=>{
  ...
  function syncAccount(){const n=q('#current-user-name')?.textContent?.trim()||'Usuario',role=q('#current-user-role')?.textContent?.trim()||'',branch=q('#current-user-branch')?.textContent?.trim()||'';const initials=n.split(/\s+/).slice(0,2).map(x=>x[0]).join('').toUpperCase()||'U';['#v219-profile-avatar','#v219-account-avatar'].forEach(s=>{if(q(s)&&q(s).textContent!==initials)q(s).textContent=initials});if(q('#v219-account-name')&&q('#v219-account-name').textContent!==n)q('#v219-account-name').textContent=n;const meta=[role,branch].filter(Boolean).join(' · ');if(q('#v219-account-meta')&&q('#v219-account-meta').textContent!==meta)q('#v219-account-meta').textContent=meta}
