/* V20.1 · Multi-línea WhatsApp + diagnóstico Suite IA */
(() => {
  let lineCatalog = [];
  let linePollTimer = null;
  let aiStatus = null;

  const lineStatusLabel = (status) => ({connected:'Conectada',qr:'Esperando QR',connecting:'Conectando',disconnected:'Desconectada',error:'Error'})[status] || status || 'Desconectada';
  const lineStatusTone = (status) => status === 'connected' ? 'ok' : status === 'qr' || status === 'connecting' ? 'wait' : status === 'error' ? 'bad' : 'off';
  ...
  async function handleLineAction(event) {
    ...
    try {
      if (action === 'connect') { await api(`/api/whatsapp-lines/${encodeURIComponent(line.id)}/connect`,{method:'POST',body:'{}'}); showToast(`Conectando ${line.name}…`); startLinePolling(); }
